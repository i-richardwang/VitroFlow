...
VitroFlow counts seeds in petri-dish photographs and turns the counts a person corrects into better detectors. Experiments are the work: dishes photographed on successive occasions and counted by one model version. The Web workbench is the control plane; independent inference and training Workers may run on different machines.
...
Photograph the dishes of an Experiment, round after round
          ↓
Inference Worker counts every photograph with the Experiment's ModelVersion
          ↓
...
| ---------------------------------------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------- |
| `models`, `model_versions`                                                   | Server           | the tasks the workbench counts (one logical Model each) and their immutable executable ModelVersions |
...
| `experiments`, `experiment_dishes`, `experiment_rounds`, `experiment_photos` | Server           | a count over the same dishes on successive occasions, fixed to one ModelVersion                      |
