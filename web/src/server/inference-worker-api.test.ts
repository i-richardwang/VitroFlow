...
test("inference HTTP routes carry an image from upload to prelabel", async () => {
  ...
  const uploadResponse = await handler(UploadRoute, "POST")({
    params: { dataset: "api" },
  ...
  const heartbeatResponse = await handler(HeartbeatRoute, "POST")({
  ...
  expect(readInferenceWorker("api-worker")?.current).toEqual(ref);

  const pendingUrl =
    "http://localhost/api/inference/pending?workerId=api-worker";
  const pendingResponse = await handler(PendingRoute, "GET")({
  ...
  expect(
    (
      await handler(PendingRoute, "GET")({
        request: new Request("http://localhost/api/inference/pending"),
      } as never)
    ).status,
  ).toBe(400);

  const imageResponse = await handler(ImageRoute, "GET")({ params: ref } as never);
  ...
  expect((await put({ ...result, source: "images/api/x.jpg" })).status).toBe(400);
...
test("an inference heartbeat cannot create an unknown model version", async () => {
  const response = await handler(HeartbeatRoute, "POST")({
    request: new Request("http://localhost/api/inference/heartbeat", {
      method: "POST",
      body: JSON.stringify({
