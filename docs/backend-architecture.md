...
`annotation-runs/runs.ts` admits single-image and batch requests under the image lock: retire expired work, check for an active run, validate the request, and freeze its definition. A batch skips active work; concurrent or repeated references cannot create duplicate active runs. Reads project expired leases without writing. `access.ts` owns authorization and lock order; `worker.ts` owns Worker claims, leases and task assignment; `tasks.ts` owns regional previews and acceptance. Pure domain operations validate geometry and collect final results. `views.ts` reads authorized image bytes and delegates rendering; MCP content encoding and task-token signing belong to `transport/mcp`. The Python Worker schedules sessions and observes server acceptance. The standalone Python runner separately owns offline file execution and export recovery.
